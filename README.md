# 👋 Hola, soy Facundo Peloso

* 🎓 Estudiante de Ingeniería en Sistemas de Información (5° año)
* 📍 Corrientes/Misiones ---> Argentina
* 💻 Apasionado por el desarrollo, la automatización y la resolución de problemas

---

## 🧠 Sobre mí

Soy estudiante avanzado de ISI con experiencia en desarrollo y tecnologías modernas.
Me adapto fácilmente a distintos entornos y desafíos, con una fuerte capacidad de análisis y resolución.
Soy una persona que tiene un fuerte interes por experiencias y desafios en sistemas organizacionles.

También cuento con experiencia técnica en armado, reparación y configuración de PCs.

---

## 🛠️ Tecnologías y herramientas

* 💻 Lenguajes: HTML, CSS, Python, Java
* ⚙️ Infraestructura: Terraform, Docker, Docker Compose
* ☁️ Cloud: AWS (S3)
* 🗄️ Bases de datos: PostgreSQL, MongoDB
* 🧩 Otros: Odoo

---

## 💼 Experiencia

### 🟢 Servicios de atención al visitante

📍 Área Cataratas del Parque Nacional Iguazú
🗓️ Ene 2026 – Feb 2026

* Atención al público en entornos de alta demanda
* Trabajo en equipo y habilidades sociales

---

### 🟢 Data Entry

📍 Desarrollos NEA
🗓️ Ago 2020 – Jun 2023

* Gestión y carga de datos
* Aplicación de metodologías ágiles

---

### 🟢 Auxiliar de RRHH

📍 Iguazú Grand Resort Spa & Casino
🗓️ Sept 2014 – Oct 2014

* Soporte en tareas administrativas
* Gestión de documentación

---

## 🚀 Intereses

* Automatización de procesos
* Desarrollo de bots
* Simulación de sistemas
* Optimización y mejora de rendimiento
* Sistemas organizacionales

---

## 📂 Próximos proyectos

🔧 Actualmente trabajando en:

* Bot de Telegram con IA
* Simulación de sistemas (eventos discretos)
* Herramientas de automatización

---
## Presentación video
[▶ Ver video](https://drive.google.com/file/d/1B1UeDvsbpFUGGZZauFlABTMdrUV8zg6r/view?usp=sharing)

---
## 📫 Contacto

* 📧 Email: pelosofacundo97@gmail.com
* 💼 LinkedIn: https://www.linkedin.com/in/facundo-peloso-3bb457223/

---
## 📄 CV
*👉 [📥 Descargar CV](https://drive.google.com/file/d/1EVuKA1AztTb1dNWft3TjEQh9s91I1jkM/view?usp=sharing)

*⭐ Siempre buscando mejorar, aprender y enfrentar nuevos desafíos.
